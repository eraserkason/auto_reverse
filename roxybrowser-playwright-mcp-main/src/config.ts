...
export function configFromCLIOptions(cliOptions: CLIOptions): Config {
  ...
  const currentHeaders = { ...(contextOptions.extraHTTPHeaders || {}) };
  if (!Object.keys(currentHeaders).some(key => key.toLowerCase() === 'accept-language'))
    currentHeaders['Accept-Language'] = 'en-US,en;q=0.9';
  if (Object.keys(currentHeaders).length)
    contextOptions.extraHTTPHeaders = currentHeaders;
  ...
  const result: Config = {
    ...
    network: {
      ...
      logRequests: cliOptions.logRequests,
      logRequestsFromFirstNavigation: cliOptions.logRequestsFromFirstNavigation,
      omitResponseBodyMimeTypes: cliOptions.omitResponseBodyMimeTypes,
    },
    logSessionId: cliOptions.logSessionId,
...
function configFromEnv(): Config {
  ...
  options.logRequests = envToBoolean(process.env.PLAYWRIGHT_MCP_LOG_REQUESTS);
  options.logRequestsFromFirstNavigation = envToBoolean(process.env.PLAYWRIGHT_MCP_LOG_REQUESTS_FROM_FIRST_NAVIGATION);
  options.logSessionId = envToBoolean(process.env.PLAYWRIGHT_MCP_LOG_SESSION_ID);
  options.omitResponseBodyMimeTypes = semicolonSeparatedList(process.env.PLAYWRIGHT_MCP_OMIT_RESPONSE_BODY_MIME_TYPES);
