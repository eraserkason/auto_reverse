...
export class Response {
  ...
  async finish() {
    ...
    if (this._includeSnapshot && this._context.currentTab())
      this._tabSnapshot = await this._context.currentTabOrDie().captureSnapshot({ waitForSurface: this.toolName === 'browser_snapshot' });
  ...
  serialize(): { content: (TextContent | ImageContent)[], isError?: boolean } {
    ...
    const sessionId = this._context.config.logSessionId ? this._context.sessionId() : undefined;

    if (sessionId) {
      response.push('### Session');
      response.push(`- Session ID: ${sessionId}`);
      response.push('');
    }
