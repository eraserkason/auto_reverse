...
import { sealSession, updateSessionMeta } from './redisWriter.js';
...
export class Context {
  ...
  private _armedNetworkLoggingUrl: string | undefined;
  ...
  private _onPageCreated(page: playwright.Page) {
    const tab = new Tab(this, page, tab => this._onPageClosed(tab), this._networkLogger);
    this._tabs.push(tab);
    if (!this._currentTab)
      this._currentTab = tab;
  }
  ...
  async dispose() {
    if (!this._disposePromise) {
      this._disposePromise = this._disposeImpl().finally(() => {
  ...
  async flushLogs() {
    await this.sessionLog?.flush();
    await this._networkLogger?.flush();
  }

  sessionId(): string | undefined {
    return this.sessionLog?.sessionId ?? this._networkLogger?.sessionId;
  }

  armNetworkLoggingForNavigation(url: string) {
    if (!this.config.network?.logRequestsFromFirstNavigation)
      return;
    if (this._networkLogger?.isActive())
      return;
    this._armedNetworkLoggingUrl = url;
    this._networkLogger?.armForNavigation();
  }

  activateArmedNetworkLogging(url: string) {
    if (!this.config.network?.logRequestsFromFirstNavigation)
      return;
    if (!this._networkLogger || this._networkLogger.isActive())
      return;
    this._armedNetworkLoggingUrl = url;
    this._networkLogger.activateArmedCapture();
    this._armedNetworkLoggingUrl = undefined;
  }

  discardArmedNetworkLogging(url?: string) {
    if (!this.config.network?.logRequestsFromFirstNavigation)
      return;
    if (!this._networkLogger || this._networkLogger.isActive())
      return;
    if (url && this._armedNetworkLoggingUrl && this._armedNetworkLoggingUrl !== url)
      return;
    this._networkLogger.discardArmedCapture();
    this._armedNetworkLoggingUrl = undefined;
  }

  private async _disposeImpl() {
    const sessionId = this.sessionId();
    try {
      if (sessionId) {
        await updateSessionMeta(sessionId, {
          status: 'flushing',
          flushingAt: new Date().toISOString(),
        });
      }
  ...
  private async _setupBrowserContext(): Promise<{ browserContext: playwright.BrowserContext, close: () => Promise<void> }> {
    ...
    // Create network logger if enabled; share sessionId with sessionLog when available
    if (this.config.network?.logRequests && !this._networkLogger) {
      const sessionId = this.sessionLog?.sessionId;
      this._networkLogger = await NetworkLogger.create(this.config, this._clientInfo.rootPath, sessionId);
    }
